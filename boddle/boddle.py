...
class boddle(object):
  ...
  def __enter__(self):
    self.stored_environ = bottle.request.environ
    bottle.request.environ = self.environ
    for k, v in self.extras.items():
      if hasattr(bottle.BaseRequest, k):
        self.extra_orig[k] = getattr(bottle.BaseRequest, k)
      setattr(bottle.BaseRequest, k, v)
    bottle.BaseRequest.app = True
    return self

  def __exit__(self, exc_type, exc_value, traceback):
    bottle.request.environ = self.stored_environ
    for k, v in self.extras.items():
      if k in self.extra_orig:
        setattr(bottle.BaseRequest, k, self.extra_orig[k])
      else:
        try:
          delattr(bottle.BaseRequest, k)
        except:
          pass
    bottle.BaseRequest.app = self.orig_app_reader