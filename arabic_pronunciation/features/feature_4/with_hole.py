...
def isFixedWord2(word):
    ...
    if wordConsonants in fixedWords:  # check if word is in the fixed word lookup table
        ...
        - If a match is found, the pronunciation is added to the `pronunciations` list and `done` is changed  to True. This needs to be done for pronounciations in the list.
        ...
        Note:
            -The `.split(u' ')` method is used to split the pronunciation strings into lists of phonemes while adding to the `pronouncians` list.
        """
        $PlaceHolder$
